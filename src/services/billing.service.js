...
const billingService = {
  ...
  async printBillToThermal(invoice, order, userId) {
    ...
    const billPrintData = {
      ...
      // ── Currency symbol: $ for Canadian taxes (HST/PST/QST), Rs otherwise ──
      currencySymbol: (() => {
  ...
  async getPendingBills(outletId, filters = {}, user = {}) {
    ...
    // Determine if user is a captain (not admin/manager/cashier/pos_user/super_admin)
    const privilegedRoles = ['master', 'admin', 'manager', 'super_admin', 'cashier', 'pos_user'];
    const userRoles = user.roles || [];
    const isCaptain = !userRoles.some(r => privilegedRoles.includes(r));
    const isCashier = userRoles.includes('cashier') || userRoles.includes('pos_user');
    ...
    if (floorId) {
      // Specific floor requested - include takeaway/delivery orders (floor_id IS NULL)
      ...
      } else if (isCaptain && user.userId) {
        // Captain: own orders on this floor + own takeaway/delivery
        whereClause += ` AND (o.floor_id = ? OR (o.floor_id IS NULL AND o.order_type IN ('takeaway', 'delivery') AND o.created_by = ?))`;
        params.push(floorId, user.userId);
      } else {
        // Admin/manager: all orders on this floor + all takeaway/delivery
        whereClause += ` AND (o.floor_id = ? OR (o.floor_id IS NULL AND o.order_type IN ('takeaway', 'delivery')))`;
        params.push(floorId);
      }
    } else if (filters.floorIds && filters.floorIds.length > 0) {
      // Multiple floors - include takeaway/delivery orders (floor_id IS NULL)
      if (isCashier && user.userId) {
        ...
        // Captain: own orders on floors + own takeaway/delivery
        whereClause += ` AND (o.floor_id IN (${filters.floorIds.map(() => '?').join(',')}) OR (o.floor_id IS NULL AND o.order_type IN ('takeaway', 'delivery') AND o.created_by = ?))`;
        params.push(...filters.floorIds, user.userId);
      } else {
        // Admin/manager: all orders on floors + all takeaway/delivery
        whereClause += ` AND (o.floor_id IN (${filters.floorIds.map(() => '?').join(',')}) OR (o.floor_id IS NULL AND o.order_type IN ('takeaway', 'delivery')))`;
  ...
  async getCaptainBills(captainId, outletId, filters = {}) {
    ...
    // Floor restriction for captain/cashier — include takeaway/delivery with floor_id IS NULL
